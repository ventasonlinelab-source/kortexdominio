(()=>{
  ...
  async function clearAudioDb(){
    try{
      const db=await openDb();
      await new Promise(resolve=>{const tx=db.transaction('blobs','readwrite');tx.objectStore('blobs').clear();tx.oncomplete=resolve;tx.onerror=resolve});
      db.close();
    }catch{}
  }
  ...
  async function clearProject(btn){
    ...
    // Prevent old one-shot seed scripts from repopulating after a deliberate clear.
    ...
    sessionStorage.removeItem('ari-last-server-recording-v1');
    await clearAudioDb();
    location.reload();
  }
  ...
  async function renderMaster(project,setProgress){
    ...
    let audioCount=0;for(const t of(project.tracks||[]))for(const c of(t.clips||[]))if(c.type==='audio'&&c.blobId)audioCount++;
    let done=0;
  ...
  async function exportMp3(btn){
    ...
    exporting=true;btn.disabled=true;const original='EXP. MP3';
    const progress=n=>btn.textContent='MP3 '+Math.max(0,Math.min(100,n))+'%';
  ...
  function mount(){
    if(!location.pathname.startsWith('/ari/music/')||document.getElementById(ROW_ID))return;
    ...
    const style=document.createElement('style');style.textContent=`
      ...
      #${ROW_ID} .actions{display:flex;align-items:center;gap:7px;padding:7px 10px;position:sticky;left:var(--headerw)}
      #${ROW_ID} button{height:32px;border:1px solid var(--line);background:var(--panel);border-radius:6px;padding:0 12px;font-size:9px;font-weight:900}
      ...
      #${ROW_ID} .export{color:var(--accent);border-color:var(--accent);background:var(--accent-soft)}
      #${ROW_ID} button:disabled{opacity:.65;cursor:wait}
    `;document.head.appendChild(style);
    const row=document.createElement('div');row.id=ROW_ID;row.innerHTML='<div class="label">PROYECTO</div><div class="actions"><button class="clear" type="button">LIMPIAR TODO</button><button class="export" type="button">EXP. MP3</button></div>';
